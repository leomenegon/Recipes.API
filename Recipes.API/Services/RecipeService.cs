...
public class RecipeService
{
    ...
    public async Task<IEnumerable<RecipeDto>> FindByNameAndDescription(string query)
    {
        if(string.IsNullOrWhiteSpace(query))
            throw new ArgumentNullException("Busca vazia");

        var results = await _data.FindByNameAndDescription(query);

        return results.Adapt<IEnumerable<RecipeDto>>();
    }
    
    public async Task<IEnumerable<RecipeDto>> FindByCommonIngredient(int commonIngredientId)
    {
        var results = await _data.FindByCommonIngredient(commonIngredientId);
